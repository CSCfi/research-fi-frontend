...
import { EXPRESS_HTTP_PORT } from './projects/mydata/src/app/app.global';
...
// Express server
const app = express();
const DIST_FOLDER = join(process.cwd(), 'dist_mydata');

// We have a routes configuration to define where to serve every app with the according language.
const routes = [
  { path: '/en/*', view: 'en/index', bundle: require('./dist_mydata/server/en/main') },
  { path: '/sv/*', view: 'sv/index', bundle: require('./dist_mydata/server/sv/main') },
  { path: '/*', view: 'fi/index', bundle: require('./dist_mydata/server/fi/main') },
];
...
} = require('./dist_mydata/server/fi/main');
const {
...
} = require('./dist_mydata/server/en/main');
const {
...
} = require('./dist_mydata/server/sv/main');
